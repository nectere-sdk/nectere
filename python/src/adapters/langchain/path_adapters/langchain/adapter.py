...
from path import ToolBase, WalletClientBase, get_tools


def get_on_chain_tools(wallet: WalletClientBase, plugins: List[Any]) -> List[BaseTool]:
    """Create LangChain tools from PATH tools.
    ...
    Returns:
        List of LangChain Tool instances configured with the PATH tools
    """
    ...
    langchain_tools = []
    for t in tools:
        # Create a LangChain Tool for each PATH tool
        tool = StructuredTool(
            name=t.name,
